...
public class TrinketsPlayerFix extends DataFix {
    public TrinketsPlayerFix(Schema schema) {
        super(schema, true);
    }

    @Override
    protected TypeRewriteRule makeRule() {
        var in = this.getInputSchema().getType(References.PLAYER);
        var out = this.getOutputSchema().getType(References.PLAYER);
